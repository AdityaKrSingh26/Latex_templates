...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \node[phase] (p1) {\textbf{Phase 1: Preprocessing} \\ {\footnotesize Filter invalid positions and sort by timestamp}};
        \node[phase, below=1cm of p1] (p2) {\textbf{Phase 2: Clustering} \\ {\footnotesize Build reachability graph and merge via handovers}};
        \node[phase, below=1cm of p2] (p3) {\textbf{Phase 3: Decision} \\ {\footnotesize Analyze significant clusters and return result}};
        
        \draw[arrow] (p1) -- (p2);
        \draw[arrow] (p2) -- (p3);
    \end{tikzpicture}
    \caption{Three-phase pipeline of the SingleVehicleCheck algorithm}
...
Figure~\ref{fig:algorithm_walkthrough} illustrates the SingleVehicleCheck algorithm applied to a concrete dataset of 6 GPS positions from 2 vehicles. We trace each position through the pipeline stages: preprocessing, clustering, graph construction, component detection, and final decision.
...
% Step 1
\node[block] (s1_title) {Step 1: Raw GPS Data (6 positions)};
\node[data, below=0.3cm of s1_title] (s1_data) {
$p_1$: Shanghai 31.23°N, 121.47°E, 08:00 \quad
$p_2$: Shenzhen 22.55°N, 114.05°E, 08:15 \\
$p_3$: Shanghai 31.24°N, 121.46°E, 09:00 \quad
$p_4$: NaN, NaN, 09:30 {\textcolor{red}{(INVALID)}} \\
$p_5$: Shenzhen 22.56°N, 114.06°E, 10:00 \quad
$p_6$: Shanghai 31.25°N, 121.48°E, 10:30
};
...
Removed: $p_4$ (NaN values) \quad Result: 5 valid positions \\
Sorted by timestamp: $p_1 \to p_2 \to p_3 \to p_5 \to p_6$
};
\draw[arrow] (s2_data.south) -- ++(0,-0.5);

% Step 3
\node[block, below=1.1cm of s2_data] (s3_title) {Step 3: Build Reachability Graph};
\node[data, below=0.3cm of s3_title] (s3_data) {
Check speed feasibility using $V_{\max} = 69.4$ m/s. \\
Shanghai cluster (Blue): $\{p_1, p_3, p_6\}$ mutually reachable. \\
Shenzhen cluster (Red): $\{p_2, p_5\}$ reachable. No cross-cluster edges.
};
\draw[arrow] (s3_data.south) -- ++(0,-0.6);

% Step 4
\node[block, below=1.3cm of s3_data] (s4_title) {Step 4: Find Connected Components};
...
\begin{tikzpicture}[>=Stealth]
    ...
    % Red cluster
    \node[cluster2] (p2) at (4,0) {$p_2$};
    \node[cluster2] (p5) at (5,0) {$p_5$};
...
Total positions after preprocessing: $n = 5$ \\
Significance threshold: $\max(5, 0.1 \times 5) = 5$ \\
Cluster sizes: 3 (Shanghai), 2 (Shenzhen) \\
...
\end{tikzpicture}
\caption{Step-by-step walkthrough of the SingleVehicleCheck algorithm on concrete GPS data from Shanghai and Shenzhen. The algorithm detects multiple vehicles and returns FALSE.}
